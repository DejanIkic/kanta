...
class PinConfig:
    ...
    def print_pin_mapping(cls):
        ...
        for pin_type, config in cls.SERVO_PINS.items():
            print(f"{config['wire_color']:12} → {config['pin_name']:8} (Pin {config['pin_number']})")
        print()
        print("UART Pinovi:")
        print(f"TX → {cls.UART_PINS['TX']['pin_name']} (Pin {cls.UART_PINS['TX']['pin_number']})")
        print(f"RX → {cls.UART_PINS['RX']['pin_name']} (Pin {cls.UART_PINS['RX']['pin_number']})")
